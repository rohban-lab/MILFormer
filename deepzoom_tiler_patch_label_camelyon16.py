### all credit to https://github.com/binli123/dsmil-wsi. We just changed some parameters
import json
...
class TileWorker(Process):
    ...
    def does_square_have_cancer(self, annotation_file, x, y, size):
        ...
        for i in coordinates:
            ...
            polyg2 = Polygon([(x, y), (x + size, y), (x, y + size), (x + size, y + size)])
...
class DeepZoomStaticTiler(object):
    ...
    def _shutdown(self):
        ...
        for _i in range(self._workers):
            print('inside loop of shutdown')
            self._queue.put(None)
        print('after loop of shutdown')
        self._queue.join()
        print('after join of shutdown')
...
if __name__ == '__main__':
    Image.MAX_IMAGE_PIXELS = None
    parser = argparse.ArgumentParser(description='Patch extraction for datasets')
    parser.add_argument('-d', '--dataset', type=str, default='camelyon16', help='Dataset name') #changed to camelyon16
    ...
    parser.add_argument('-v', '--slide_format', type=str, default='tif', help='Image format for tiles [svs]') #changed to tif
    parser.add_argument('-j', '--workers', type=int, default=8, help='Number of worker processes to start [4]') #12 because i have many cores
    parser.add_argument('-q', '--quality', type=int, default=75, help='JPEG compression quality [70]') #i changed to 75 because it's the default of pillow
    parser.add_argument('-s', '--tile_size', type=int, default=256, help='Tile size [224]') #i changed to 256 becuase can be better compared with other methods i think
    parser.add_argument('-b', '--base_mag', type=float, default=20, help='Maximum magnification for patch extraction [20]')
    parser.add_argument('-m', '--magnifications', type=int, nargs='+', default=[1], help='Levels for patch extraction [0]') #changed to 1
    parser.add_argument('-o', '--objective', type=float, default=20, help='The default objective power if metadata does not present [20]')
    parser.add_argument('-t', '--background_t', type=int, default=20, help='Threshold for filtering background [15]')  #changed to 20
