# some parts are ours some are from https://github.com/AMLab-Amsterdam/AttentionDeepMIL and some from https://github.com/binli123/dsmil-wsi
...
from sklearn.metrics import roc_curve, roc_auc_score, auc, average_precision_score, f1_score
...
def multi_label_roc(labels, predictions, num_classes, for_feats=False):
    ...
    for c in range(0, num_classes):
        ...
        # c_auc = roc_auc_score(fpr, tpr)
        ap = average_precision_score(label, prediction)  # II
        binary_feat_prediction = np.where(prediction >= threshold_optimal, 1, 0)  # III
        f1 = f1_score(label, binary_feat_prediction)  # III
    ...
    return aucs, thresholds, thresholds_optimal, ap_scores, f1_scores
...
def _models_and_weights_to_json(i_classifier, embedder_state_dict_weights, milnet, aggregator_state_dict_weights):
    with open('i_classifier.json', 'w') as f:
        keys = [str(key) for key in i_classifier.state_dict().keys()]
        json.dump(keys, f)
    with open('embedder_weights.json', 'w') as f:
        keys = [str(key) for key in embedder_state_dict_weights.keys()]
        json.dump(keys, f)

    with open('milnet.json', 'w') as f:
        keys = [str(key) for key in milnet.state_dict().keys()]
        json.dump(keys, f)
    with open('aggregator.json', 'w') as f:
        keys = [str(key) for key in aggregator_state_dict_weights.keys()]
        json.dump(keys, f)
